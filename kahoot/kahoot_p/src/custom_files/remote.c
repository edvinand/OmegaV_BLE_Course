...
int simple_ad_start(char option)
{
    int err = 0;
    LOG_INF("Initializing answer %c", option);
...
int set_adv_name(uint8_t * p_name, uint8_t length)
{
    if (length != 4) {
        return -EINVAL;
    }
